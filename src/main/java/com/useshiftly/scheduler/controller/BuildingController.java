...
import com.useshiftly.scheduler.dto.BuildingDTO;
...
public class BuildingController {
    ...
    public ResponseEntity<?> addManagerToBuilding(@PathVariable Long buildingId, @RequestBody Map<String, Object> request) {
        ...
        com.useshiftly.scheduler.model.Employee manager = employeeRepository.findById(managerId).orElse(null);
        if (manager == null) {
            return ResponseEntity.status(404).body(Map.of("error", true, "message", "Manager employee not found"));
        }
        if (manager.getRole() != com.useshiftly.scheduler.model.Employee.Role.MANAGER) {
