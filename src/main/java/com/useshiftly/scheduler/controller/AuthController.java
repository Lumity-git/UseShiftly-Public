...
import com.useshiftly.scheduler.security.JwtUtils;
import com.useshiftly.scheduler.service.EmployeeService;
import com.useshiftly.scheduler.service.InvitationService;
...
public class AuthController {
    ...
    public ResponseEntity<?> registerAdmin(
            @Valid @RequestBody com.useshiftly.scheduler.dto.auth.AdminRegisterRequest request) {
        try {
            if (employeeService.getEmployeeByEmail(request.getOwnerEmail()).isPresent()) {
                return ResponseEntity.badRequest().body(new MessageResponse("Error: Email is already taken!"));
            }
            // Create building first
            var building = new com.useshiftly.scheduler.model.Building();
            building.setName(request.getBuildingName());
            building.setAddress(request.getBuildingAddress());
            com.useshiftly.scheduler.model.Building savedBuilding = buildingRepository.save(building);
    ...
    public ResponseEntity<?> changePassword(@AuthenticationPrincipal Employee employee,
            @Valid @RequestBody com.useshiftly.scheduler.dto.auth.ChangePasswordRequest request) {
    ...
    public ResponseEntity<?> validateInvitation(@RequestParam String code, @RequestParam String token) {
        try {
            ...
            return ResponseEntity.ok(java.util.Map.of(
                    ...
                    "organizationName", "Grand Workspace"));
        } catch (Exception e) {
            return ResponseEntity.badRequest()
     ...
     * management for UseShiftly teams.
     * 
     * Key Endpoints:
